...
const freiburgDaytrips = {
  ...
  triberg: trip({
    ...
    highlights: [["Triberg Waterfalls", "黑森林最具辨識度的瀑布景點，重點是選一段舒服的步道，而不是完成所有階梯。", "瀑布、森林、階梯", mapsSearch("Triberg Waterfalls")], ["Schwarzwaldbahn", "火車本身就是景觀體驗，可把沿途山谷視作回程放鬆段。", "鐵路、山谷、交通", mapsSearch("Schwarzwaldbahn")]],
  ...
  schiltach: trip({
    ...
    activityOptions: [["Old Town Walk", "由市場廣場、木骨架街道到河邊，適合慢走和拍攝。", "低", mapsSearch("Schiltach Old Town")], ["Schüttesäge Museum", "了解 Kinzig 河谷的木材和鋸木傳統，開放時間需確認。", "低｜需查開放", mapsSearch("Schüttesäge Museum")], ["Kinzig riverside", "在河邊看小鎮立面和橋樑，適合午餐後消化。", "低", mapsSearch("Kinzig river Schiltach")]],
  ...
  todtnau: trip({
    ...
    highlights: [["Todtnauer Wasserfall", "南黑森林最直接的瀑布景觀，適合和 Triberg 作南北兩種瀑布選擇比較。", "瀑布、山谷、短步道", mapsSearch("Todtnauer Wasserfall")], ["Hasenhorn", "把瀑布自然日延伸成夏季山地活動，但高度依賴營運和天氣。", "夏季活動、山景、滑車", mapsSearch("Hasenhorn Todtnau")]],
...
function renderFreiburgDaytrip(data) {
  ...
  document.querySelector("#freiburg-daytrip-page").innerHTML = `
    ...
    <section><h2>交通與互動地圖</h2><div id="freiburg-trip-map" class="daytrip-map" aria-label="${escapeHtml(data.title)} 地圖"></div><div class="daytrip-map-legend"><span><i class="legend-map-number transit">T</i>交通节点</span><span><i class="legend-map-number">1</i>景点</span><span><i class="legend-map-number restaurant">R</i>餐厅</span></div><p class="small"><strong>橙色虚线：</strong>${escapeHtml(data.transitLabel)}；<strong>蓝色实线：</strong>${escapeHtml(data.localLabel)}。</p></section>
